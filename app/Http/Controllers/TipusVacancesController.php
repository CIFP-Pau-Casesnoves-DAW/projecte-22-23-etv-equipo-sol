...
class TipusVacancesController extends Controller
{
    ...
    public function updateTipusVacance(Request $request){
        ...
        $tipusvacances=TipusVacances::find($request->ID);

        if ($tipusvacances == null){
            return response()->json(["Status" => "Error","Result"=> "No existeix cap tipus de vacances amb aquesta id"], 400);
        }
